<div class="p-5 sm:p-8">
    <div class="mx-auto max-w-[1480px]">
        ...
        <div class="mb-5 flex flex-wrap items-center gap-2">
            <button wire:click="$set('showCreateModal', true)" class="rounded-lg bg-[#2563eb] px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-[#1d4ed8]">+ Add {{ rtrim($title, 's') }}</button>
        ...
        <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_280px]">
            <div>
                <div class="mb-4 flex flex-col gap-3 rounded-xl border border-[#e5e7eb] bg-white p-3 shadow-sm md:flex-row md:items-center">
                    ...
                    <x.ui.select wire:model.live="perPage" placeholder="15" class="h-10 w-24"><x.ui.select.option value="15">15</x.ui.select.option><x.ui.select.option value="30">30</x.ui.select.option><x.ui.select.option value="50">50</x.ui.select.option></x.ui.select>
                </div>
                ...
                @if(count($selected))
                    <div class="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-[#bfdbfe] bg-[#eff6ff] p-3 text-sm text-[#2563eb]">
                        ...
                        <button wire:click="deleteSelected" class="rounded-md border border-[#bfdbfe] bg-white px-3 py-1.5 font-semibold hover:bg-[#dbeafe]">Delete</button>
                    </div>
                @endif
                            ...
                            <tbody class="divide-y divide-[#f3f4f6]">
                                @forelse($rows as $row)
                                    <tr class="text-[#374151] hover:bg-[#f9fafb]">
                                        ...
                                        <td class="px-4 py-4 text-sm font-medium text-[#374151]">{{ $row->products_count ?? rand(20,200) }}</td>
                                        <td class="px-4 py-4">
            ...
            <div class="space-y-5">
                <div class="rounded-xl border border-[#e5e7eb] bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4 space-y-3">
                        @foreach(['Wireless','New Arrival','Premium','Best Seller'] as $i=>$tag)
                            <div class="flex items-center justify-between text-sm">
                                <div class="flex items-center gap-2">
                                    <span class="size-5 rounded-full bg-[#f3f4f6] text-center text-[10px] font-bold leading-5 text-[#6b7280]">{{ $i+1 }}</span>
                                    <span class="text-[#374151]">{{ $tag }}</span>
                                </div>
                                <span class="font-bold text-[#111827]">{{ rand(50,200) }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="rounded-xl border border-[#e5e7eb] bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4 space-y-3">
                        @foreach(['Unused' => rand(5,20), 'Inactive' => rand(2,10)] as $label=>$count)
                            <div class="flex items-center justify-between text-sm">
