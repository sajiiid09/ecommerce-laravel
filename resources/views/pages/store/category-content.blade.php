...
<main x-data="{ filtersOpen: false }" class="bg-store-soft py-6 sm:py-8">
    <x-store.ui.container>
        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <h1 class="text-2xl font-extrabold tracking-tight text-store-ink sm:text-3xl">{{ $categoryName }}</h1>
        ...
        <form method="GET" class="mt-6 grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
            ...
            <aside id="category-filters" x-cloak x-show="filtersOpen || window.innerWidth >= 1024" x-transition
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3"><a href="{{ route('store.category', ['slug' => $slug]) }}"
                    ...
                    <fieldset>
                        ...
                        <div class="mt-3 space-y-2">
                            @foreach ($categories as $category)
                                <a wire:navigate href="{{ route('store.category', ['slug' => $category['slug']]) }}"
                                    class="block text-sm {{ ($slug ?? null) === $category['slug'] ? 'font-bold text-store-blue' : 'text-store-text hover:text-store-blue' }}">{{ $category['name'] }}</a>
                            @endforeach
                        </div>
                    </fieldset><x-store.ui.filter-group title="Brands" name="brand" :options="$brands"
                        :selected="$selectedBrands" /><x-store.ui.filter-group title="Availability" name="in_stock"
                        :options="['In stock only']" :selected="request('in_stock') ? ['In stock only'] : []" />
                </div>
            ...
            <section>
                <div
                    ...
                    <p class="text-sm text-store-muted">Showing <span
                            class="font-bold text-store-ink">{{ count($products) }}</span> products</p>
                ...
                <div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
                    ...
                    @endforeach
                </div><x-store.ui.pagination :current="$page" :total="$pageCount" />
            </section>
        </form>
    </x-store.ui.container>
</main>
