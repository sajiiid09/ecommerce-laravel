    ...
    <main class="bg-white pb-10">
        <x-store.ui.container>
            <div class="grid items-stretch gap-4 pt-5 lg:grid-cols-[220px_minmax(0,1fr)_280px]">
                <aside class="hidden h-full rounded-card border border-store-border bg-white p-3 lg:block">
                    @foreach ($categoryNames as $index => $name)
                        <a href="{{ route('store.category') }}" wire:navigate
                            class="flex items-center justify-between border-b border-store-border px-2 py-3 text-sm last:border-0 hover:text-store-blue"><span
                                class="flex min-w-0 items-center gap-2"><x-ui.icon :name="$categoryIcons[$index]"
                                    class="size-4 shrink-0 !text-store-blue" /><span
                                    class="truncate">{{ $name }}</span></span><x-ui.icon name="chevron-right"
                                class="size-4 !text-store-muted" /></a>
                    @endforeach
                </aside>

                <x-store.ui.carousel loop autoplay label="Featured promotions" :show-dots="false" :show-controls="false"
                    ...
                    <article
                        ...
                        <div class="relative z-10 max-w-xl">
                            ...
                            <p class="mt-4 max-w-md text-base text-blue-100">Groceries, fashion, electronics & more at
                                unbeatable prices.</p><a href="{{ route('store.offers') }}" wire:navigate
                        ...
                        </div><x-ui.icon name="shopping-bag"
                            class="pointer-events-none absolute right-10 top-10 size-32 !text-blue-200/30 sm:size-40" />
                    </article>
                    <article
                        ...
                        {{-- <x-ui.icon name="sparkles"
                            class="pointer-events-none absolute right-10 top-10 size-32 !text-red-100/30 sm:size-40" /> --}}
                    </article>
                    <article
                        ...
                        <div class="relative z-10 max-w-xl">
                            ...
                            <p class="mt-4 max-w-md text-base text-emerald-100">Stock up on groceries and home favorites
                                ...
                                Groceries</a>
                        </div><x-ui.icon name="shopping-cart"
                            class="pointer-events-none absolute right-10 top-10 size-32 !text-emerald-100/30 sm:size-40" />
                    </article>
                    {{-- <div class="pointer-events-none absolute bottom-7 left-7 z-20 flex gap-2 sm:bottom-10 sm:left-10"
                        aria-label="Featured promotion slides"><template x-for="page in pageCount"
                            :key="page"><button type="button"
                                class="pointer-events-auto size-2 rounded-full transition-colors"
                                :class="current === page - 1 ? 'bg-white' : 'bg-white/50'" @click="goTo(page - 1)"
                                :aria-label="`Go to featured promotion ${page}`"></button></template></div> --}}
                </x-store.ui.carousel>
            ...
            <section id="category" class="mt-8">
                ...
                <x-store.ui.carousel loop label="Shop by category" :show-dots="false">
                    @foreach ($categoryNames as $index => $name)
                        <article class="w-[calc((100%-0.75rem)/2)] shrink-0 sm:w-[calc((100%-1.5rem)/3)] md:w-[calc((100%-2.25rem)/4)] xl:w-[calc((100%-3.75rem)/6)]">
                            <a href="#category"
                                class="block rounded-card border border-store-border bg-white p-3 text-center transition hover:-translate-y-0.5 hover:shadow-store-soft"><span
                                    class="mx-auto flex aspect-square w-full items-center justify-center rounded-control bg-store-soft px-2 text-center"><x-ui.icon
                                        :name="$categoryIcons[$index]" class="size-8 !text-store-blue" /></span><span
                                    class="mt-2 block text-xs font-semibold text-store-ink">{{ $name }}</span></a>
                        </article>
                    @endforeach
                </x-store.ui.carousel>
            </section>
            ...
            <section id="offers" class="mt-8">
                ...
                <x-store.ui.carousel loop label="Flash sale products" :show-dots="false">
                    @foreach (array_slice($products, 3, 6) as $product)
                        <div class="w-[calc((100%-0.75rem)/2)] shrink-0 md:w-[calc((100%-1.5rem)/3)] xl:w-[calc((100%-3rem)/5)]"><x-store.catalog.product-card :product="$product" compact /></div>
                    @endforeach
                </x-store.ui.carousel>
            </section>
                        ...
                        <article class="flex items-center gap-3 rounded-card border border-store-border bg-white p-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs font-bold text-store-success">Save
                                    ৳{{ number_format($combo['save']) }}</p><button type="button"
            ...
            <section class="mt-8">
                ...
                <x-store.ui.carousel loop label="Best seller products" :show-dots="false">
                    @foreach (array_slice($products, 9, 6) as $product)
                        <div class="w-[calc((100%-0.75rem)/2)] shrink-0 md:w-[calc((100%-1.5rem)/3)] xl:w-[calc((100%-3rem)/5)]"><x-store.catalog.product-card :product="$product" compact /></div>
                    @endforeach
                </x-store.ui.carousel>
            </section>
            ...
            <section id="brands" class="mt-8">
                <div class="mb-4 flex items-center justify-between">
                    <h2 class="text-xl font-extrabold tracking-tight text-store-ink">Top Brands You Trust</h2><a
                        ...
                        â†’</a>
                --}}
                </div><x-store.ui.carousel loop label="Top brands">
                    @foreach ($brands as $brand)
                        <article
                            class="w-[calc((100%-0.75rem)/2)] shrink-0 sm:w-[calc((100%-2.25rem)/4)] lg:w-[calc((100%-3.75rem)/6)]">
                            <a href="#brands"
                                class="grid min-h-16 h-full place-items-center rounded-card border border-store-border bg-white p-3 text-center text-sm font-black text-store-blue transition hover:border-store-blue">{{ $brand }}</a>
                        </article>
                    @endforeach
                </x-store.ui.carousel>
            </section>
            ...
            <section class="mt-8">
                ...
                <x-store.ui.carousel loop label="Fresh picks" :show-dots="false">
                    @foreach (array_slice($products, 3, 6) as $product)
                        <div class="w-[calc((100%-0.75rem)/2)] shrink-0 md:w-[calc((100%-1.5rem)/3)] xl:w-[calc((100%-3rem)/5)]"><x-store.catalog.product-card :product="$product" compact /></div>
                    @endforeach
                </x-store.ui.carousel>
            ...
            <section class="mt-8">
                ...
                </div><x-store.ui.carousel loop label="Customer testimonials"><x-store.ui.testimonial-card
                        class="w-[calc((100%-0rem)/1)] shrink-0 snap-start sm:w-[calc((100%-0.75rem)/2)] lg:w-[calc((100%-1.5rem)/3)]"
                        name="Nusrat Jahan"
                        quote="Great experience! Fast delivery and products were exactly as described. StoreZ is my new go-to shop." /><x-store.ui.testimonial-card
                        class="w-[calc((100%-0rem)/1)] shrink-0 snap-start sm:w-[calc((100%-0.75rem)/2)] lg:w-[calc((100%-1.5rem)/3)]"
                        name="Rafiq Ahmed"
                        quote="Love the combo deals and COD option. Very convenient and trustworthy platform." /><x-store.ui.testimonial-card
                        class="w-[calc((100%-0rem)/1)] shrink-0 snap-start sm:w-[calc((100%-0.75rem)/2)] lg:w-[calc((100%-1.5rem)/3)]"
                        name="Tania Rahman"
                        quote="Quality products, best prices and excellent customer service. Highly recommended." /><x-store.ui.testimonial-card
                        class="w-[calc((100%-0rem)/1)] shrink-0 snap-start sm:w-[calc((100%-0.75rem)/2)] lg:w-[calc((100%-1.5rem)/3)]"
                        name="Farhan Kabir"
                        quote="The product selection is excellent and delivery updates made every order easy to follow." /><x-store.ui.testimonial-card
                        class="w-[calc((100%-0rem)/1)] shrink-0 snap-start sm:w-[calc((100%-0.75rem)/2)] lg:w-[calc((100%-1.5rem)/3)]"
                        name="Maliha Sultana"
                        quote="I found exactly what I needed at a fair price, and the checkout was quick and simple." /><x-store.ui.testimonial-card
                        class="w-[calc((100%-0rem)/1)] shrink-0 snap-start sm:w-[calc((100%-0.75rem)/2)] lg:w-[calc((100%-1.5rem)/3)]"
                        name="Imran Hossain"
                        quote="Fast service, helpful support, and reliable packaging. I’ll definitely shop from StoreZ again." /></x-store.ui.carousel>
            </section>
        </x-store.ui.container>
    </main>
</x-layouts.app>
