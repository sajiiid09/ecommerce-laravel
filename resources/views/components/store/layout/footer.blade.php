...
<footer class="bg-store-navy text-slate-200">
    <x-store.ui.container class="py-10">
        <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div><h2 class="text-base font-bold text-white">Popular Categories</h2><ul class="mt-3 space-y-2 text-sm">@foreach (array_slice($categories, 0, 4) as $category)<li><a href="#{{ $category['slug'] }}" class="hover:text-white">{{ $category['name'] }}</a></li>@endforeach</ul></div>
