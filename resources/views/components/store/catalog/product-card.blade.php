...
<article {{ $attributes->class('group relative flex min-h-full flex-col overflow-hidden rounded-card border border-store-border bg-white transition duration-200 hover:-translate-y-0.5 hover:shadow-store-soft') }}>
    <div class="relative {{ $compact ? 'p-2 sm:p-3' : 'p-4' }}">
        ...
        @php
            $image = $product['image'] ?? asset('images/placeholders/no-image.svg');
            $imageUrl = str($image)->startsWith(['http://', 'https://', '/']) ? $image : asset(ltrim($image, '/'));
        @endphp
        <a href="{{ route('store.product', ['slug' => $product['slug']]) }}" wire:navigate class="block aspect-square"><img src="{{ $imageUrl }}" alt="{{ $product['name'] }}" loading="lazy" decoding="async" class="size-full object-contain transition duration-200 group-hover:scale-[1.03]"></a>
    </div>
    <div class="flex flex-1 flex-col px-3 pb-3 sm:px-4 sm:pb-4">
        ...
        @if ($product['isVariable'] ?? false)
            <button type="button" class="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-control bg-store-blue px-3 text-sm font-bold text-white transition hover:bg-store-blue-dark disabled:cursor-not-allowed disabled:opacity-50" :disabled="cartAddPending" @click="openProductQuickAdd(@js($product))"><x-ui.icon name="shopping-cart" class="size-4 !text-white" />Add to Cart</button>
        @else
            <button type="button" class="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-control bg-store-blue px-3 text-sm font-bold text-white transition hover:bg-store-blue-dark disabled:cursor-not-allowed disabled:opacity-50" :disabled="cartAddPending" @click="addToCart(@js($product))"><x-ui.icon name="shopping-cart" class="size-4 !text-white" />Add to Cart</button>
        @endif
    </div>
</article>
