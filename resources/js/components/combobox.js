const comboboxComponent = ({
    ...
    return {
        ...
        init() {
            ...
            this.optionClickHandler = (event) => {
                const target = event.target;
                const closestOption = target instanceof Element
                    ? target.closest('[x-rover\\:option]')
                    : null;

                if (!closestOption || !this.$el.contains(closestOption)) return;

                const value = closestOption.dataset.value ?? closestOption.getAttribute('value');
                if (value === null) return;

                event.preventDefault();
                event.stopPropagation();
                this.handleSelection(String(value));
            };

            this.$el.addEventListener('click', this.optionClickHandler, true);
        ...
        destroy() {
            ...
            if (this.optionClickHandler) {
                this.$el.removeEventListener('click', this.optionClickHandler, true);
            }
        },
    };
};
