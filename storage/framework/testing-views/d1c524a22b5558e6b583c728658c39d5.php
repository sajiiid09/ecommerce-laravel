...
<?php endif; ?><div><h1 class="text-2xl font-extrabold text-store-ink sm:text-3xl">My Orders</h1><p class="mt-1 text-sm text-store-muted">View and track your StoreZ orders.</p><div class="mt-6 space-y-4"><article class="rounded-card border border-store-border bg-white p-5"><div class="flex flex-wrap items-center justify-between gap-3"><div><p class="text-sm font-bold text-store-ink">Order #SZ-100248</p><p class="mt-1 text-xs text-store-muted">Placed August 20, 2026 · 2 items · ৳5,180</p></div><?php if (isset($component)) { $__componentOriginal756bb0dc222e9d9d77c2d621d89b7cdf = $component; } ?>
...
<?php endif; ?></div><div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-store-border pt-4"><p class="text-sm text-store-muted">Expected delivery: <strong class="text-store-ink">August 24, 2026</strong></p><a href="<?php echo e(route('account.tracking', ['order' => 'SZ-100248'])); ?>" wire:navigate class="inline-flex h-10 items-center rounded-control bg-store-blue px-4 text-sm font-bold text-white">Track Order</a></div></article></div></div></div> <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?></main>
<?php /**PATH C:\xampp\htdocs\storez\resources\views/pages/account/orders-content.blade.php ENDPATH**/ ?>