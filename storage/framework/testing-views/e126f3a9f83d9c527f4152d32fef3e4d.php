...
<?php endif; ?>
            <div>
                <nav class="mb-5 text-xs text-store-muted"><a href="<?php echo e(route('account.orders')); ?>" wire:navigate
                    ...
                    <div class="relative ml-2 border-l-2 border-store-blue pl-7 sm:ml-5 sm:pl-10">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $timeline; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $event): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                            <div class="relative pb-8 last:pb-0"><span
                                    class="absolute -left-[2.05rem] grid size-8 place-items-center rounded-full <?php echo e($event['active'] ?? false ? 'bg-store-blue text-white' : ($event['completed'] ?? false ? 'bg-green-100 text-store-success' : 'bg-store-soft text-store-muted')); ?> sm:-left-[3.05rem]">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($event['active'] ?? false): ?>
...
<?php endif; ?>
                                    <?php elseif($event['completed'] ?? false): ?>
                                    <?php if (isset($component)) { $__componentOriginal56804098dcf376a0e2227cb77b6cd00a = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal56804098dcf376a0e2227cb77b6cd00a = $attributes; } ?>
...
<?php endif; ?>
                                ...
                                <h2 class="text-sm font-bold text-store-ink"><?php echo e($event['label']); ?></h2>
                                <p class="mt-1 text-xs text-store-muted">
                                    <?php echo e($event['active'] ?? false ? 'Today, 9:00 AM' : ($event['completed'] ?? false ? 'Order update confirmed' : 'Awaiting delivery update')); ?>

                                </p>
                                <p class="mt-2 text-sm text-store-text">
                                    <?php echo e($event['active'] ?? false ? 'The delivery partner is heading to you.' : ($event['completed'] ?? false ? 'Your order has progressed to this stage.' : 'We will notify you when this step is complete.')); ?>
...
<?php /**PATH D:\projects\laravel\storez\resources\views/pages/account/tracking-content.blade.php ENDPATH**/ ?>