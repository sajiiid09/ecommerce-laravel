...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.store.ui.breadcrumb','data' => ['items' => [['label' => $categoryName]]]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['items' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute([['label' => $categoryName]])]); ?>
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
...
<?php endif; ?>
        <div class="flex flex-wrap items-end justify-between gap-4"><div><h1 class="text-2xl font-extrabold tracking-tight text-store-ink sm:text-3xl"><?php echo e($categoryName); ?></h1><p class="mt-1 text-sm text-store-muted">Browse quality products at everyday StoreZ prices.</p></div><button type="button" class="inline-flex h-10 items-center gap-2 rounded-control border border-store-blue bg-white px-4 text-sm font-bold text-store-blue lg:hidden" @click="filtersOpen = true" aria-controls="category-filters" :aria-expanded="filtersOpen.toString()"><?php if (isset($component)) { $__componentOriginal56804098dcf376a0e2227cb77b6cd00a = $component; } ?>
...
<?php endif; ?></button></div></div>
                <div class="mt-5 space-y-5"><fieldset><legend class="text-sm font-bold text-store-ink">Categories</legend><div class="mt-3 space-y-2"><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?><a wire:navigate href="<?php echo e(route('store.category', ['slug' => $category['slug']])); ?>" class="block text-sm <?php echo e(($slug ?? null) === $category['slug'] ? 'font-bold text-store-blue' : 'text-store-text hover:text-store-blue'); ?>"><?php echo e($category['name']); ?></a><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?></div></fieldset><?php if (isset($component)) { $__componentOriginaleadf2e85c5e8c348d9a095e7163fe50c = $component; } ?>
...
<?php endif; ?></div>
            ...
            <section><div class="flex flex-wrap items-center justify-between gap-3 rounded-card border border-store-border bg-white p-3"><p class="text-sm text-store-muted">Showing <span class="font-bold text-store-ink"><?php echo e(count($products)); ?></span> products</p><div class="flex items-center gap-2"><label for="sort-products" class="text-xs font-semibold text-store-muted">Sort by</label><select id="sort-products" name="sort" onchange="this.form.submit()" class="h-10 rounded-control border border-store-border bg-white px-3 text-sm text-store-ink"><option value="popular" <?php if($sort === 'popular'): echo 'selected'; endif; ?>>Popularity</option><option value="price_asc" <?php if($sort === 'price_asc'): echo 'selected'; endif; ?>>Price: Low to High</option><option value="price_desc" <?php if($sort === 'price_desc'): echo 'selected'; endif; ?>>Price: High to Low</option><option value="rating" <?php if($sort === 'rating'): echo 'selected'; endif; ?>>Rating</option></select></div></div><div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4"><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = array_slice($products, ($page - 1) * 12, 12); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?><?php if (isset($component)) { $__componentOriginalc573f75add0e58aeac1c3f6ba4c16330 = $component; } ?>
...
<?php /**PATH C:\xampp\htdocs\storez\resources\views/pages/store/category-content.blade.php ENDPATH**/ ?>